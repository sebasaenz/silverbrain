...
import { ItemData, MEMOTEST_DUMMY_DATA, generateMemotestGridData } from '../../constants/memotest'
...
const Row: React.FC<RowProps> = ({ row, handleItemPress, selectedItems, guessedItems, disabled, isTabletOrMobileDevice }) => {
    return (
...
const Item: React.FC<ItemProps> = ({ item, onPress, selectedItems, guessedItems, disabled, isTabletOrMobileDevice }) => {
    ...
    return (
        <TouchableOpacity onPress={() => !disabled && onPress(item.key!)} style={[styles.item, {backgroundColor}, {
            width: isTabletOrMobileDevice ? 80 : 110, height: isTabletOrMobileDevice ? 80 : 110
        }]}>
            <Text
                style={
                    [
...
const Memotest: React.FC<Record<string, never>> = () => {
    ...
    useEffect(() => {
        ...
        if (selectedItems.length == 2) {
            const firstSelectedTitle = dataWithKeys.current[Math.ceil(selectedItems[0] / 4 - 1)][(selectedItems[0] - 1) % 4].title
            const secondSelectedTitle = dataWithKeys.current[Math.ceil(selectedItems[1] / 4 - 1)][(selectedItems[1] - 1) % 4].title

            if (firstSelectedTitle == secondSelectedTitle) {
                if (selectedItems.length + guessedItems.length
                        == dataWithKeys.current.map((el: ItemData[]): number => el.length).reduce((el: number, acc: number) => acc + el)) {
    ...
    return (
        <View>
            ...
            <View style={{display: duration > 0 ? undefined : 'none'}}>
                ...
                <Text>Right guesses: {rightGuesses} / {rightGuesses + wrongGuesses}</Text>
            </View>
        </View>
    )
}
